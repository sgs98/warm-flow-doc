...
| 功能                                                | 功能解释                              |
|---------------------------------------------------|-----------------------------------|
| [通过](./api.md#流程通过)                               | 当前节点处理人，对当前流程节点进行审核通过，完成后进入下一节点   |
| [回退](./api.md#流程退回)                               | 当前节点处理人，将流程驳回至之前已经处理过的任务节点，要求重新处理 |
| [任意通过](./api.md#流程任意通过)                           | 任意通过到指定节点                         |
| [任意退回](./api.md#流程任意退回)                           | 任意退回到指定节点                         |
| [驳回到上一个任务](./api.md#驳回上一个任务 )                     | 任务直接驳回到，最后一次跳转到当前任务的前置任务。         |
| [超时自动审批](../enhance/listenerAndNode.md#_1、超时自动审批) | 超过一定时间自动审批通过或者退回。                 |
| [拿回](./api.md#根据insid拿回到最近办理的任务)                  | 拿回到当前人最近办理的任务                     |
| [撤销](./api.md#撤销)                                 | 将工作流退回给发起人审批，以便进行修改或重新处理          |
| [终止](./api.md#根据流程实例id终止流程)                       | 当前节点处理人，终止当前流程                    |
...
| [暂存](./api.md#根据流程实例id暂存任务)                       | 复杂表单，一次性填写不完，需要保存草稿功能，开始节点的暂存     |
