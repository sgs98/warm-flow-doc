...
public class VariableTest {
    ...
    public void testVariable() {
        ...
        log.info("spel办理人表达式结果:{}", ExpressionUtil.evalVariable("#{@user.evalVarEntity(#handler)}", variable1));
...
## 3、匹配规则
- 1、默认按照注入策略顺序，倒序匹配。比如最后注入spel策略，就先遍历spel策略，匹配上就执行。
